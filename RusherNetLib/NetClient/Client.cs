...
namespace RusherNetLib.NetClient {
    public sealed class Client : BaseClient {
        ...
        public override IClient AddHandler(ClientType type, ClientHandler handler) {
            switch (type) {
                case ClientType.Connected: OnConnected += handler;
                    break;
                case ClientType.Sended: OnSended += handler;
                    break;
                case ClientType.Received: OnReceived += handler;
                    break;
                case ClientType.Disconnected: OnDisconnected += handler;
                    break;
            }
            return this;
        }
        ...
        internal override void InvokeEvent(ClientType type, IConnection conn, IMessage msg) {
            switch (type) {
                case ClientType.Connected: OnConnected?.Invoke(conn, msg);
                    break;
                case ClientType.Sended: OnSended?.Invoke(conn, msg);
                    break;
                case ClientType.Received: OnReceived?.Invoke(conn, msg);
                    break;
                case ClientType.Disconnected: OnDisconnected?.Invoke(conn, msg);
                    break;
            }
        }
